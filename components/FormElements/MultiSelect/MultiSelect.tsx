...
const MultiSelect: React.FC<DropdownProps> = ({ id, courses, selected, setSelected, values, error }) => {
        ...
        <div className="relative z-50">
            ...
            <div>
                <select className="hidden" id={id}>
                    {Object.keys(courses)
                        .filter(key => isNaN(Number(courses[key as keyof typeof courses])))
                        .map((key) => (
                            <option key={key} value={courses[key as keyof typeof courses]}>
                ...
                {error.courses ? (
                    <p className="text-red">{error.courses}</p>
                ) : null}
            </div>
        </div>
    );
