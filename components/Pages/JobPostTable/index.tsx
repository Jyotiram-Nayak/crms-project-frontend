...
import {
  DateFilter,
  truncateText,
} from "@/components/Filters/DataFilter/DataFilter";
import {
  ToastError,
  ToastSuccess,
...
const JobPostTable: React.FC = () => {
  ...
  const onDeleteStudent = async (jobId: string) => {
    try {
      ...
      if (response.payload?.success) {
        ToastSuccess(response.payload?.message);
        setJobs((prevValue) => prevValue.filter((job) => job.jobId !== jobId));
      } else if (response.error?.message) {
        ToastError(response.error.message || "An error occurred.");
      }
    } catch (error) {
                  ...
                  {jobs &&
                    jobs.map((job, index) => (
                      <tr key={index}>
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p className="text-black dark:text-white">
                            {job.postedDate ? DateFilter(job.postedDate) : "--"}
                          </p>
                        </td>
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                          <p className="text-black dark:text-white">
                        ...
                        {role !== "Student" && (
                          <>
                            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                              <p className="text-black dark:text-white">
                                {job.approvedDate ? DateFilter(job.approvedDate) : "--"}
                              </p>
                            </td>
                            <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                              <button
                        ...
                        {role === "Student" && (new Date(job.deadline) > new Date() ?(
                          <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p className="text-black dark:text-white">
                              <ApplyButton
                        ...
                        <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                            <p  className="bg-red rounded font-medium gap-2.5 hover:bg-opacity-90 items-center px-2 py-2 text-white flex justify-center "  style={{width:"95px"}}>
                              Expired
                            </p>
                          </td>)}
                        {role === "Company" && (
